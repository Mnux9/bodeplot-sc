...
blocks = 50 # Number of 1024 samples to capture
...
ap.add_argument( "--fstart", type = int, default=20, help = "Starting frequency for the bodeplot in Hz. Default 10 Hz." )
...
#set chart to the top one
plt.subplot(2, 1, 1)
graph = plt.plot(frequency,gain)[0]
plt.xlim(options.fstart,options.fstop)
plt.xscale("log")
plt.yscale("log")
plt.grid(True, which="both", linestyle="--", linewidth=0.5)
plt.title("Magnitude frequency response")
plt.ylabel("Gain magnitude (log scale)")
plt.xlabel("Frequency [Hz]")

plt.subplot(2, 1, 2)
graph = plt.plot(frequency,meow)[0]
plt.xlim(options.fstart,options.fstop)
plt.ylim(180,-180)
plt.xscale("log")
plt.grid(True, which="both", linestyle="--", linewidth=0.5)
...
while(freq < options.fstop):
    ...
    # Wait x for things to settle out with the function generator and scope
    time.sleep(0.08)
    ...
    #update plots
    plt.subplot(2, 1, 1)
    gain.append(rms1/rms2)
    frequency.append(freq)
    #plot gain in teal
    graph = plt.plot(frequency, gain, color = 'teal')[0] #frre

    plt.subplot(2, 1, 2)
    #plot gain in teal
    meow.append(deltaphase*180/math.pi)
    graph = plt.plot(frequency, meow, color = 'teal')[0] #frre
